...
To mark a field as required it's necessary to add block 'required_fields' to the 'mailer' page part.

Simple example of a required field (mailer part):

  required_fields:
    - name

Simple example of a required field with user-defined message (mailer part):
...
Simple example of a required field with email address validation (mailer part):

  required_fields:
    - reply_email: as_email

You can also specify fields which must be validated as defined regexp (i.e. /^\d{2}\.\d{2}\.\d{4}\$/ for date dd.mm.yyyy).

Simple example of a required field with regexp validation (mailer part):

  required_fields:
    - birthday: /^\d{2}\.\d{2}\.\d{4}\$/


=== Required fields (OLD IMPLEMENTATION)
